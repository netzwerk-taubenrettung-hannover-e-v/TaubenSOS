...
class Case(db.Model):
    ...
    def get_breed(latNE, lonNE, latSW, lonSW, fromTime=None, untilTime=None):
        if fromTime is not None and untilTime is not None:
            sql = text('select sum(case when breed = :feralPigeon then 1 else 0 end) as "feralPigeon", sum(case when breed = :fancyPigeon then 1 else 0 end) as "fancyPigeon", sum(case when breed = :carrierPigeon then 1 else 0 end) as "carrierPigeon", sum(case when breed = :commonWoodPigeon then 1 else 0 end) as "commonWoodPigeon", sum(case when breed is Null then 1 else 0 end) as "undefined" from "case" where latitude between :latSW and :latNE and longitude between :lonSW and :lonNE and timestamp between :fromTime and :untilTime')
            ...
            sql = text('select sum(case when breed = :feralPigeon then 1 else 0 end) as "feralPigeon", sum(case when breed = :fancyPigeon then 1 else 0 end) as "fancyPigeon", sum(case when breed = :carrierPigeon then 1 else 0 end) as "carrierPigeon", sum(case when breed = :commonWoodPigeon then 1 else 0 end) as "commonWoodPigeon", sum(case when breed is Null then 1 else 0 end) as "undefined" from "case" where latitude between :latSW and :latNE and longitude between :lonSW and :lonNE and timestamp > :fromTime')
            ...
            sql = text('select sum(case when breed = :feralPigeon then 1 else 0 end) as "feralPigeon", sum(case when breed = :fancyPigeon then 1 else 0 end) as "fancyPigeon", sum(case when breed = :carrierPigeon then 1 else 0 end) as "carrierPigeon", sum(case when breed = :commonWoodPigeon then 1 else 0 end) as "commonWoodPigeon", sum(case when breed is Null then 1 else 0 end) as "undefined" from "case" where latitude between :latSW and :latNE and longitude between :lonSW and :lonNE and timestamp < :untilTime')
            sql = sql.bindparams(latSW=latSW, latNE=latNE, lonSW=lonSW, lonNE=lonNE, fromTime=fromTime, untilTime=untilTime, feralPigeon="Feral Pigeon", fancyPigeon="Fancy Pigeon", commonWoodPigeon="Common Wood Pigeon", carrierPigeon="Carrier Pigeon")
        else:
            sql = text('select sum(case when breed = :feralPigeon then 1 else 0 end) as "feralPigeon", sum(case when breed = :fancyPigeon then 1 else 0 end) as "fancyPigeon", sum(case when breed = :carrierPigeon then 1 else 0 end) as "carrierPigeon", sum(case when breed = :commonWoodPigeon then 1 else 0 end) as "commonWoodPigeon", sum(case when breed is Null then 1 else 0 end) as "undefined" from "case" where latitude between :latSW and :latNE and longitude between :lonSW and :lonNE')
