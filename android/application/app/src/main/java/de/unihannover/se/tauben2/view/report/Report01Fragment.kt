...
class Report01Fragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val case: Case = arguments?.getParcelable("case") ?: Case(null, latitude = 0.0, longitude = 0.0)
        ...
        view.report_next_step_button.setOnClickListener {
            ...
            if (canGoForward(case)) {
                case.priority = report_state_seekbar.progress
                val bundle = Bundle()
                bundle.putParcelable("case", case)
                Navigation.findNavController(context as Activity, R.id.nav_host).navigate(R.id.report02Fragment, bundle)
            } else {
                context?.let { c ->
    ...
    private fun canGoForward(case: Case): Boolean {

        val injuries = BooleanArray(report_injury_layout.childCount)
        var check = false
